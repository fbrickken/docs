...
Every write follows the standard two-step flow: **prepare** (unsigned transaction) → **send** (`POST /send-transactions`). Relayed preparation is free; client-controlled RAMS operations split the x402 charge between prepare and send. Unlike the ERC-8004 facades, `/x402/rams/*` facades are **client-signed by default** — most RAMS operations require `msg.sender` to hold a specific role, so Brickken's relayer cannot send them. Only the four signed lifecycle operations can be relayed, and only with a principal EIP-712 signature.
...
<CardGroup cols={2}>
  ...
  <Card title="Pricing" icon="badge-dollar-sign" href="/api-reference/endpoint/agentic-pricing">
    Relayed prepare is free; client-controlled modes split the x402 fee between prepare and send.
  </Card>
</CardGroup>
