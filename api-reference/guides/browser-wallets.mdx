...
const raw = Array.isArray(response.transactions)
...
<Note>
  The `Array.isArray` check is deliberate. Most methods return `transactions` as an array, but
  [`mintToken`](/api-reference/endpoint/prepare-mintToken) returns a single object. Handle both.
</Note>
