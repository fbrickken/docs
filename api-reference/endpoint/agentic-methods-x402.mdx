...
description: 'ERC-8004 agent methods exposed through x402scan-discoverable facades and prepare-transactions, with API key or x402 payment authentication.'
---

Agentic methods are ERC-8004 and agent-token operations. x402scan discovers them as semantic facade endpoints under `/x402/...`; each facade prepares the same unsigned transaction as `POST /prepare-transactions`, but fixes the `method` from the path.

1. Call one facade endpoint such as `POST /x402/token/create`, or call `POST /prepare-transactions` directly.
2. Sign the returned transaction locally.
3. Call `POST /send-transactions`.
4. Poll `GET /get-transaction-status`.

Private keys never leave the client. These methods can be authenticated with either an `x-api-key` header or an x402 payment. Non-agentic methods still require `x-api-key`. x402scan lists only the prepare facades; `send-transactions` remains the follow-up submission step because it requires the `txId` and signed transaction returned by prepare.

## x402 Authentication

For x402, omit `x-api-key` and call either a `/x402/...` facade or `POST /prepare-transactions`. If payment is required, the API returns `402 Payment Required` with a `PAYMENT-REQUIRED` response header containing a base64-encoded x402 payment request.
...
Use production (`https://api.brickken.com`) as the public x402scan target. Sandbox (`https://api.sandbox.brickken.com`) exposes the same routes for QA. `GET /.well-known/x402` returns the absolute resource URLs, and `GET /openapi.json` serves OpenAPI 3.1 with `x-payment-info` on each facade.

Facade requests use the same body fields as the matching method, except the `method` field is optional and ignored because the path fixes it.

| Facade endpoint | Fixed method |
| --- | --- |
...
| Method | Purpose |
| --- | --- |
| `newTokenizedAgent` | Legacy alias for agent registration; not published in x402scan discovery |
...
| `agentTransferToken` | Transfer an agent token |
| `agentTransferFromToken` | Transfer an agent token through allowance |
| `agentApproveToken` | Approve an agent token allowance |

## Common Prepare Request

CLI and advanced integrations can prepare all methods through `POST /prepare-transactions` by passing `method` explicitly.
...
For x402scan and simple resource discovery, call the semantic facade instead and omit `method`:
...
For x402, omit `x-api-key`. If the first response is `402`, retry with `X-Payment`.
...
Use `POST /send-transactions` after signing the prepared transaction. This endpoint is not listed as a separate x402scan resource because it depends on the `txId` and signed payload produced by the prepare step.
...
`newTokenizedAgent` is a legacy alias that uses the same request body as `agentRegister`. It is supported for direct `/prepare-transactions` integrations, but it is not published in the public x402scan discovery list.
...
Transfers tokens by calling `transfer(to, amount)` on the deployed agent token.

Required fields:

| Field | Type | Description |
| --- | --- | --- |
| `method` | string | Must be `agentTransferToken` when using `/prepare-transactions` |
| `ownerEmail` or `email` | string | Tokenizer email |
| `signerAddress` | string | Wallet that owns the tokens being transferred |
| `tokenAddress` | string | Deployed ERC-20 token address |
| `to` | string | Recipient wallet |
| `amount` | string | Human-readable amount to transfer |
...
```json
{
  ...
  "ownerEmail": "owner@example.com",
  "signerAddress": "0xTokenHolderWallet",
  "tokenAddress": "0xDeployedTokenAddress",
  "to": "0xRecipientAddress",
  "amount": "25",
  "decimals": 18
...
Transfers tokens through allowance by calling `transferFrom(from, to, amount)` on the deployed agent token.

Required fields:

| Field | Type | Description |
| --- | --- | --- |
| `method` | string | Must be `agentTransferFromToken` when using `/prepare-transactions` |
| `ownerEmail` or `email` | string | Tokenizer email |
| `signerAddress` | string | Spender wallet that has allowance |
| `tokenAddress` | string | Deployed ERC-20 token address |
...
```json
{
  ...
  "ownerEmail": "owner@example.com",
  "signerAddress": "0xApprovedSpenderWallet",
  "tokenAddress": "0xDeployedTokenAddress",
  "from": "0xSourceWallet",
  "to": "0xRecipientAddress",
  "amount": "25",
...
Approves a spender by calling `approve(spenderAddress, amount)` on the deployed agent token.

Required fields:

| Field | Type | Description |
| --- | --- | --- |
| `method` | string | Must be `agentApproveToken` when using `/prepare-transactions` |
| `ownerEmail` or `email` | string | Tokenizer email |
| `signerAddress` | string | Token owner wallet |
| `tokenAddress` | string | Deployed ERC-20 token address |
| `spenderAddress` | string | Wallet receiving allowance |
| `amount` | string | Human-readable allowance amount |
...
```json
{
  ...
  "ownerEmail": "owner@example.com",
  "signerAddress": "0xTokenOwnerWallet",
  "tokenAddress": "0xDeployedTokenAddress",
  "spenderAddress": "0xSpenderWallet",
  "amount": "25",
  "decimals": 18
}
```
