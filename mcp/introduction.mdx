...
The Brickken MCP server exposes the Brickken Dapp API and Agentic API as 57 tools for MCP-compatible AI agents.

It is **the client to reach for when an AI agent should drive the full Dapp API with nothing but an API key** — 13 read tools and 11 write tools that go through `prepare-transactions`. The CLI cannot do this at all; if your own code is driving rather than an agent, use the [SDK](/sdk/introduction). See [Choose your surface](/get-started/choose-your-surface).
...
Use this mode for the [Dapp API](/api-reference/introduction). An API key alone unlocks every Dapp tool listed below — tokenization, STOs, security-token operations, and all reads.
