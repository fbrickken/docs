...
The **Brickken CLI** is the public terminal client for KYC onboarding and the [Agentic API](/api-reference/endpoint/agentic-methods-x402). It supports ERC-8004 on Base and ERC-8226 RAMS mandates on Ethereum Sepolia. In relayed mode it prepares writes for free and pays the full operation through x402 at send; client-controlled modes split the operation charge between prepare and send. Agent getters and RAMS reads use API-key authentication or x402.
...
High-level commands are prepare-only by default. Relayed preparation is free; client-controlled preparation charges the first half of the operation. Add `--execute` to also send and pay the remaining x402 charge in one step. Add `--json` for machine-readable output.
