...
| --- | --- | :---: | :---: | :---: | :---: |
| Create a tokenized asset | `newTokenization` | Yes | Yes | Yes | No |
| Mint tokens | `mintToken` | Yes | Yes | Yes | No |
| Burn tokens | `burnToken` | Yes | Yes | Yes | No |
| Whitelist or remove an investor | `whitelist` | Yes | Yes | Yes | No |
| Transfer tokens | `transferTo` | Yes | Yes | Yes | No |
| Transfer through an allowance | `transferFrom` | Yes | Yes | Yes | No |
| Approve an allowance | `approve` | Yes | Yes | Yes | No |
| Distribute dividends | `dividendDistribution` | Yes | Yes | Yes | No |
...
| Capability | Method | HTTP | SDK | MCP | CLI |
| --- | --- | :---: | :---: | :---: | :---: |
| Launch an offering | `newSto` | Yes | Yes | Yes | No |
| Take an investment | `newInvest` | Yes | Yes | Yes | No |
| Let investors claim their tokens | `claimTokens` | Yes | Yes | Yes | No |
| Close the offering | `closeOffer` | Yes | Yes | Yes | No |
...
| --- | --- | :---: | :---: | :---: | :---: |
| Chain metadata and contract addresses | `GET /get-network-info` | Yes | Yes | Yes | No |
| Token information | `GET /get-token-info` | Yes | Yes | Yes | No |
| Tokenizer account and token contract address | `GET /get-tokenizer-info` | Yes | Yes | Yes | No |
| List offerings for a token | `GET /get-stos` | Yes | Yes | Yes | No |
| One offering by ID | `GET /get-sto-by-id` | Yes | Yes | Yes | No |
| Offering balance and status | `GET /get-sto-balance` | Yes | Yes | Yes | No |
| Investments in an offering | `GET /get-investments-by-sto-id` | Yes | Yes | Yes | No |
| Investor status | `GET /get-investor-info` | Yes | Yes | Yes | No |
| Whitelist status of a wallet | `GET /get-whitelist-status` | Yes | Yes | Yes | No |
| Balance and whitelist together | `GET /get-balance-whitelist` | Yes | Yes | Yes | No |
| Allowance between two parties | `GET /get-allowance` | Yes | Yes | Yes | No |
| Dividend distribution | `GET /get-dividend-distribution` | Yes | Yes | Yes | No |
| Transaction status | `GET /get-transaction-status` | Yes | Yes | Yes | Yes |

<Note>
The **No** column for the CLI is not an oversight. The CLI is built for agentic workflows and has no commands for the Dapp methods. Use the [SDK](/sdk/introduction), HTTP, or MCP for everything above — see [Choose your surface](/get-started/choose-your-surface).
</Note>
...
- **The CLI cannot call the general Dapp API.** It supports KYC link creation, agent getters, and RAMS reads, but not tokenization, STO, or their Dapp reads. The [SDK](/sdk/introduction) is the client to use for full Dapp work from your own code.
