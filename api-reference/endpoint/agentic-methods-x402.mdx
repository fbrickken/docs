...
1. **Prepare** the unsigned transaction. In `brickken-relayed` mode this is free; in `client-signed` and `client-broadcast` modes the operation charge is split between prepare and send.
...
1. **Prepare.** Call a semantic facade such as `POST /x402/agent/register`, or `POST /prepare-transactions` with an eligible `method`. For relayed execution, pass `executionMode: "brickken-relayed"`. The response returns `txId`, `transactions`, and an `x402Requirements` quote. Relayed preparation is free; client-controlled preparation may return a payment challenge for its share of the operation price.
