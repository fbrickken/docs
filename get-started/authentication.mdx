...
### CLI — KYC and supported reads

<Warning>
**The CLI cannot drive the general legacy Dapp API.** KYC link creation is the explicit supported exception. The CLI sends `x-api-key` for KYC and, when configured, for agent getters and RAMS reads/typed-data fetches. It still has no commands for tokenization, STO, or their Dapp read methods. `brickken tx prepare --method newTokenization` forwards the method name but the backend rejects it with `API key is required for this method`.
