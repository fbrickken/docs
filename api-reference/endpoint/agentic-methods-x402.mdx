...
| `executionMode` | string | No | `brickken-relayed` (Brickken signs and broadcasts) or `client-signed`. **The default depends on the facade**: `/x402/agent/*` and `/x402/token/*` inject `brickken-relayed`; `/x402/rams/*` default to `client-signed`. Calling `/prepare-transactions` directly defaults to `client-signed`. |
