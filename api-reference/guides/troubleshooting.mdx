...
<Note>
  ...
  whitelist entry, or the offering's lifecycle stage. It is never a compliance verdict. The KYC
  gate on `newInvest` runs *after* gas estimation, so if the chain state is wrong you see the
  revert first and the friendlier KYC message never appears. Fix the chain state, then re-run; if
  KYC is genuinely missing, you will get a plain `400` at that point.
</Note>
...
| `approve` returns `Bad to address` | A malformed `spenderAddress` | Despite the wording, this validates `spenderAddress`. There is no `to` field on this method. The usual cause is a leading or trailing space from a copied value or an environment variable, or a bad EIP-55 checksum. Trim it, or send it all lowercase. |
...
| `mintToken` succeeds but your client reports that no transaction came back | `mintToken` returns `transactions` as a **single object**, not an array | Handle both shapes. See [`mintToken`](/api-reference/endpoint/prepare-mintToken). |
| A later call reports the token or balance does not exist, although a transaction confirmed | An earlier transaction in a multi-transaction response was never broadcast | When a response contains `whitelistTx`/`txIdWhitelist` alongside `transactions`/`txId`, sign and send **both, whitelist first**. Skipping the first leaves the second on an unusable nonce. |
