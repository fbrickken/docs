---
title: 'CLI'
description: 'Public Brickken CLI for x402-paid agentic workflows and ERC-8226 RAMS mandates.'
---

The **Brickken CLI** is the public terminal client for the [Agentic API](/api-reference/endpoint/agentic-methods-x402). It supports ERC-8004 on Base and ERC-8226 RAMS mandates on Ethereum Sepolia. It prepares writes for free and pays the send through x402; RAMS reads and typed-data fetches use API-key authentication.
...
Writes use x402. RAMS reads and online EIP-712 typed-data fetches use an API key.
...
<Warning>
**The CLI does not reach the legacy Dapp API.** It has no commands for tokenization, STOs, or the Dapp reads, and it only attaches `x-api-key` to RAMS reads. For those workflows use the [MCP server](/mcp/introduction) or the [REST API](/api-reference/introduction) — see [Choose your surface](/get-started/choose-your-surface).
</Warning>
